...
`./`               | `nerocash.conf`       | User-defined configuration settings for `nerocashd` or `nerocash-qt`; can be specified by `-conf` option
`./`               | `nerocashd.pid`        | Stores the process ID (PID) of `nerocashd` or `nerocash-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `nerocashd` or `nerocash-qt`
