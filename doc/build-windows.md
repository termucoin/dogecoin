...
Below are some notes on how to build NonceCash Core for Windows.
...
To build NonceCash from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the NonceCash GitHub repository](https://github.com/noncecash/noncecash/)).
...
Note that for WSL v1 the NonceCash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/noncecash, AND not under, for example, /mnt/d/noncecash.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for NonceCash's dependencies):
```bash
...
way. This will install to `c:\workspace\noncecash`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/noncecash
```
