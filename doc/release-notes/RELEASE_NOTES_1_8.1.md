# NonceCash Core 1.8.1

NonceCash Core 1.8.1 is primarily a bugfix release, bringing NonceCash Core in
line with Bitcoin 0.9.3. NonceCash Core 1.8.1 also adds in support for printing
...
## NonceCash Core Release Notes

* Translation improvements and corrections
* Disable Transifex based translation process (not used by NonceCash Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 NonceCash Core clients
* Use HTTPS with nnccchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/noncecash/noncecash/issues/134).
...
This only affects values significantly below 1 Nncc, however merchants may wish to round to
the nearest Nncc on all transactions for now.
