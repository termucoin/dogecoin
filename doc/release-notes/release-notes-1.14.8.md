NonceCash Core version 1.14.8 is now available from:

  <https://github.com/noncecash/noncecash/releases/tag/v1.14.8/>

This is a new minor version release. NonceCash users on any platform are
...
Several important updates have been added to NonceCash Core, including fixes
backported from Bitcoin Core's upstream.

### Reduce Peer INV Object Counts

Previously, NonceCash Core nodes would maintain a large list of inventory for
...
A new transaction request tracking system improves the way NonceCash Core
...
In previous versions of NonceCash Core, orphan transactions were processed in a
...
NonceCash Core now sends a getheaders message for one block when announced in an
...
vulnerability or security issue in NonceCash Core.
...
* NonceCash Core now supports compilation with C++14 and C++17, if you select
